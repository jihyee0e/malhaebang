...
def getVector(news_df):
    category_names = ["금융", "증권", "산업/재계", "중기/벤처", "부동산", "글로벌 경제", "생활경제", "경제 일반"]
    vector_list = []
...
def addClusterNumber(news_df, vector_list):
    # news_df['cluster_number'] = -1
    # for category, vector in vector_list:
    #     model = DBSCAN(eps=0.3, min_samples=2, metric='cosine')
    #     result = model.fit_predict(vector)
    #     idxs = news_df[news_df['category'] == category].index

    #     if len(result) != len(idxs):
    #         print(f"⚠️ 군집 결과 개수 불일치: {category}")
    #         continue

    #     for i, idx in enumerate(idxs):
    #         news_df.at[idx, 'cluster_number'] = result[i]
    cluster_number_list = []
